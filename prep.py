...
# function to create a sql url using credentials of user
def get_connection(db, username=username, host=host, password=password):
    '''
   Function uses credentials from env file to
    create a connection url to access the MySQL Codeup db.
    '''
    return f'mysql+pymysql://{username}:{password}@{host}/{db}'



# function to get zillow data from codeup MySQL database and returns as a pandas DataFrame
def get_zillow_data():
    '''
    Function reads in the zillow data from the Codeup db,
    selects all columns from the properties_2017 table,
    joins predictions_2017 table,
    and acquires single unit properties with transactions during May 2017 - August 2017 'hot months'
    ...
    #read in dataframe from Codeup MySQL database
